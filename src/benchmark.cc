...
std::unique_ptr<BenchmarkReporter> CreateReporter(
  ...
  if (name == "console") {
    ...
    //std::exit(1);
    Rcpp::stop("Unexpect format: ", name, "\n");
  }
}
...
bool ReportUnrecognizedArguments(int argc, char** argv) {
  for (int i = 1; i < argc; ++i) {
    /*fprintf(stderr,*/ REprintf("%s: error: unrecognized command-line flag: %s\n", argv[0], argv[i]);
  }
  return argc > 1;
}
