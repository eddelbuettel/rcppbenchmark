...
void InitializeSystemInfo() {
  ...
  if (num_cpus == 0) {
    /*fprintf(stderr,*/ REprintf("Failed to read num. CPUs correctly from /proc/cpuinfo\n");
  } else {
...
//  139         error = sysctl_handle_int(oidp, &freq, sizeof(freq), req);
  ...
  if (sysctlbyname(sysctl_path, &hz, &sz, nullptr, 0) != 0) {
    /*fprintf(stderr,*/ REprintf("Unable to determine clock rate from sysctl: %s: %s\n",
...
// TODO: also figure out cpuinfo_num_cpus
  ...
  if (::sysctlbyname("hw.cpufrequency", &cpu_freq, &size, nullptr, 0) == 0 &&
    ...
    /*fprintf(stderr,*/ REprintf("CPU frequency cannot be detected. \n");
